...
- Relying on the CSP as the control. The CSP lives in `vercel.json`, the loader lives in `server/_shared/brief-render.js`, and the loader's comment argued for exactly the change that would remove the protection. The block was incidental, not intentional.
...
The fix also moves the control into the same file as the thing it controls. The renderer now carries the reason in its comment and a test that fails when the attribute is removed, so the protection survives a future CSP edit that admits `abacus.worldmonitor.app`.

## Prevention

- A page whose URL carries a credential must not load an analytics tracker without query exclusion. The default behavior of Umami, and of most trackers, is to report `location.search`. First-party and self-hosted trackers are not exempt, because the credential lands in a queryable database either way. Check this on every bearer link whose query carries the credential, including the public-share mirror `api/brief/public/[hash].ts`, which is covered by the same `/api/brief/(.*)` CSP entry.
- A guard that lives in a different file than the thing it guards is not a control. The `script-src` list in `vercel.json:569-572` blocked the loader while the loader's own comment in `server/_shared/brief-render.js` argued the tracker belonged there. Put the protection next to the emitting code, state the reason in the comment, and pin it with a test that fails on removal.
